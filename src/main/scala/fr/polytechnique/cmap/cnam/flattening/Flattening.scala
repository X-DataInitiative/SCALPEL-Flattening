...
object Flattening {
  ...
  def saveCSVTablesAsParquetAndORC(sqlContext: SQLContext, conf: FlatteningConfig): List[OperationMetadata] = {
          ...
          list.map {
            config: ConfigPartition =>
              ...
              if (config.partitionColumn.isDefined) {
                typedTable.writeParquetAndORC(config.output, config.partitionColumn.get)(
                  config.singleTableSaveMode,
                  config.fileFormat
                )
              } else {
                typedTable.writeParquetAndORC(config.output)(config.singleTableSaveMode, config.fileFormat)
              }
  ...
  def joinSingleTablesToFlatTable(sqlContext: SQLContext, conf: FlatteningConfig): List[OperationMetadata] = {
    conf.joinTableConfigs.filter(_.saveFlatTable).map { config =>
      if (config.pmsiPatientTableName.isEmpty) {
        ...
        new FlatTable(sqlContext, config).writeAsParquetAndORC()
        OperationMetadata(
          config.name, config.flatOutputPath.get, "flat_table",
        ...
        if (config.joinKeysPatient.isEmpty) {
          ...
          new PMSIFlatTable(sqlContext, config).writeAsParquetAndORC()
          OperationMetadata(
            config.name, config.flatOutputPath.get, "flat_table",
            config.mainTableName :: (config.tablesToJoin ++ config.refsToJoin.map(_.name)), config.joinKeys
          )
          ...
          new SSRRIPFlatTable(sqlContext, config).writeAsParquetAndORC()
          OperationMetadata(
            config.name, config.flatOutputPath.get, "flat_table",
            config.mainTableName :: (config.tablesToJoin ++ config.refsToJoin.map(_.name)), config.joinKeys
          )
