...
object FlatteningMain extends Main {
  ...
  def run(sqlContext: SQLContext, argsMap: Map[String, String]): Option[Dataset[_]] = {
    ...
    val metaConvert = saveCSVTablesAsSingleTables(sqlContext, conf)
    ...
    // Write Metadata
    val metadata = MainMetadata(this.getClass.getName, startTimestamp, new java.util.Date(), metaConvert ++ metaJoin)
